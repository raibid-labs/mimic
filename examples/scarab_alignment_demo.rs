...
async fn main() -> ratatui_testlib::Result<()> {
    ...
    use ratatui_testlib::{
        AsyncTuiTestHarness,
        events::KeyCode,
        navigation::{NavigationTestExt, NavMode},
    };
    ...
    harness.send_text("echo 'Links: [a] Google [b] GitHub'; echo '-- VISUAL --'\r").await?;
    ...
    harness.wait_for_async(|state| {
        state.contents().contains("-- VISUAL --")
    }).execute().await?;
    ...
    harness.send_text("sleep 1; echo 'Build Complete'\r").await?;
